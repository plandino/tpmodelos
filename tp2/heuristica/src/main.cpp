...
int main(int argc, char* argv[]){
	...
	int cantidadConflictos = 3;
	int cantidadDeSectores = 4;
	int vicepresidencia;
	...
	int minimaCantidad = cantidadConflictos;
	...
	for(int i = 0; i < cantidadDeSectores; i++){
	...
	int minCantidadConflictosEncontrados = cantidadConflictos;
	...
	for(int i = 0; i != sectorAResolver and i < cantidadDeSectores; i++){
		...
		if( posturasSectorAResolver.size() <= conflictosSector.size() ){
			for(unsigned int j = 0; j < posturasSectorAResolver.size(); j++){
				for(unsigned int l = 0; l < conflictosSector.size(); l++){
					for(unsigned int k = 0; k < cantidadConflictos; k++){
